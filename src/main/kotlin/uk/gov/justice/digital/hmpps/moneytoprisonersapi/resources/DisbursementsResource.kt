...
import uk.gov.justice.digital.hmpps.moneytoprisonersapi.dto.Disbursement
...
class DisbursementsResource(
  ...
  fun listDisbursements(
    ...
    principal: Principal,
  ): PaginatedResponse<Disbursement> {
    val disbursements = disbursementService.listDisbursements(
      ...
      created = created?.let { java.time.OffsetDateTime.parse(it + "T00:00:00") },
      createdGte = createdGte?.let { java.time.OffsetDateTime.parse(it + "T00:00:00") },
      createdLt = createdLt?.let { java.time.OffsetDateTime.parse(it + "T00:00:00") },
      loggedAtGte = loggedAtGte?.let { java.time.OffsetDateTime.parse(it + "T00:00:00") },
      loggedAtLt = loggedAtLt?.let { java.time.OffsetDateTime.parse(it + "T00:00:00") },
      logAction = logAction,
    ...
    val results = disbursements.map { Disbursement.from(it, nameMap) }
  ...
  @PreAuthorize("isAuthenticated()")
  @GetMapping("/{id}/")
  fun getDisbursement(@PathVariable id: Long): ResponseEntity<Disbursement> {
    val disbursement = disbursementService.getDisbursement(id)
      ?: return ResponseEntity.notFound().build()
    return ResponseEntity.ok(Disbursement.from(disbursement, prisonNameMap()))
  }
  ...
  fun createDisbursement(
    ...
    return Disbursement.from(disbursement, prisonNameMap())
  }
  ...
  fun updateDisbursement(
    ...
    ResponseEntity.ok(Disbursement.from(updated, prisonNameMap()))
