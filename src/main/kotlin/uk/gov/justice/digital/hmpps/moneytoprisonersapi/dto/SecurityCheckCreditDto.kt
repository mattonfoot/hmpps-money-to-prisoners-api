...
@Schema(description = "A credit record with security details and security check information")
data class SecurityCheckCreditDto(
  ...
  // Security check (CRD-108)
  @Schema(description = "Security check details")
  @JsonProperty("security_check")
  val securityCheck: SecurityCheckDto?,
) {
  companion object {
    fun from(
      credit: Credit,
    ...
    ): SecurityCheckCreditDto = SecurityCheckCreditDto(
      id = credit.id,
      ...
      creditedAt = credit.logs.firstOrNull { it.action == LogAction.CREDITED.value }?.created,
      refundedAt = credit.logs.firstOrNull { it.action == LogAction.REFUNDED.value }?.created,
      setManualAt = credit.logs.firstOrNull { it.action == LogAction.MANUAL.value }?.created,
      ...
      senderIpAddress = credit.payment?.ipAddress?.hostAddress,
