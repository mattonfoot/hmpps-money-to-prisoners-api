...
interface FileDownloadRepository : JpaRepository<FileDownload, Long> {
  ...
  @Query("SELECT MIN(f.date) FROM CoreFiledownload f WHERE f.label = :label")
  fun findEarliestDateByLabel(@Param("label") label: String): LocalDate?

  @Query("SELECT f.date FROM CoreFiledownload f WHERE f.label = :label AND f.date IN :dates")
