...
class FileDownloadServiceTest {
  ...
  inner class FindMissingDownloads {
    ...
    fun `COR-003 returns dates not present in the database after earliest record`() {
      ...
      whenever(fileDownloadRepository.findEarliestDateByLabel(label)).thenReturn(LocalDate.of(2024, 1, 13))
      whenever(
        fileDownloadRepository.findDatesByLabelAndDateIn(
          label,
          listOf(LocalDate.of(2024, 1, 13), LocalDate.of(2024, 1, 14), LocalDate.of(2024, 1, 15)),
        ),
    ...
    fun `COR-003 excludes dates before earliest record to avoid false positives`() {
      ...
      whenever(fileDownloadRepository.findEarliestDateByLabel(label)).thenReturn(LocalDate.of(2024, 1, 14))
      whenever(
        fileDownloadRepository.findDatesByLabelAndDateIn(
          label,
          listOf(LocalDate.of(2024, 1, 13), LocalDate.of(2024, 1, 14), LocalDate.of(2024, 1, 15)),
    ...
    @Test
    fun `returns empty list when no records exist for label`() {
      val label = "bank_statement"

      whenever(fileDownloadRepository.findEarliestDateByLabel(label)).thenReturn(null)

      val missing = service.findMissingDownloads(label, listOf("2024-01-13", "2024-01-14"))
