...
import uk.gov.justice.digital.hmpps.moneytoprisonersapi.jpa.entities.InvalidCreditResolutionException
...
class CreditServiceTest {
  ...
  private fun makePrison(nomisId: String): Prison = Prison().apply {
    this.nomisId = nomisId
  }

  private fun makeUser(username: String, id: Long? = null): AuthUser = AuthUser().apply {
    this.username = username
    this.id = id
  }

  private fun toOffset(ldt: LocalDateTime?): OffsetDateTime? = ldt?.atOffset(ZoneOffset.UTC)

  /**
   * Test fixture builder for Credit. The parked tests used `prison: String?` and
   * `owner: String?` — translate those to entity references here. `receivedAt`
   * is LocalDateTime in the test for convenience; converted to OffsetDateTime
   * on assignment.
   *
   * `incompleteSenderInfo` lives on Transaction in the JPA model. When provided
   * (true or false where a transaction context is needed), we create a stub
  ...
  private fun createCredit(
    ...
    owner: String? = null,
    incompleteSenderInfo: Boolean? = null,
  ): Credit {
    val c = Credit().apply {
      this.id = id
      this.amount = amount
      ...
      this.prison = prison?.let { makePrison(it) }
      this.resolution = resolution.value
      this.blocked = blocked
      this.reviewed = reviewed
      this.reconciled = reconciled
      this.receivedAt = toOffset(receivedAt)
      this.owner = owner?.let { makeUser(it) }
    }
    if (incompleteSenderInfo != null) {
      c.transaction = Transaction().apply {
  ...
  inner class CreateCredit {
    ...
    fun `saves and returns a new credit`() {
      ...
      val result = creditService.createCredit(
        ...
        receivedAt = OffsetDateTime.of(2024, 3, 15, 10, 30, 0, 0, ZoneOffset.UTC),
    ...
    fun `defaults resolution to initial`() {
      ...
      assertThat(captor.firstValue.resolution).isEqualTo(CreditResolution.INITIAL.value)
    }
  }

  // -----------------------------------------------------------------------
  // listCredits filter tests — small helpers used widely.
  // -----------------------------------------------------------------------

  private val completedResolutions = listOf(CreditResolution.INITIAL.value, CreditResolution.FAILED.value)

  private fun stubCompletedCredits(vararg credits: Credit) {
    whenever(creditRepository.findByResolutionNotIn(completedResolutions)).thenReturn(credits.toList())
  }

  private fun stubAllCredits(vararg credits: Credit) {
    whenever(creditRepository.findAll()).thenReturn(credits.toList())
  }
  ...
  private fun makeTransaction(
    ...
    if (senderSortCode != null) this.senderSortCode = senderSortCode
    if (senderAccountNumber != null) this.senderAccountNumber = senderAccountNumber
    if (senderRollNumber != null) this.senderRollNumber = senderRollNumber
  ...
  private fun makePayment(
    ...
    this.email = email
    this.ipAddress = ipAddress?.let { InetAddress.getByName(it) }
  ...
  private fun makeBillingAddress(postcode: String): BillingAddress = BillingAddress().apply {
    this.postcode = postcode
  }

  private fun makeSecurityCheck(
    credit: Credit,
  ...
  private fun makeLog(
    ...
    this.credit = credit
    this.created = createdAt.atOffset(ZoneOffset.UTC)
    this.user = user
  }
  ...
  inner class ListCreditsFiltered {
    ...
    fun `returns all completed credits when no filters`() {
      ...
      whenever(creditRepository.findByResolutionNotIn(completedResolutions)).thenReturn(credits)
    ...
    fun `filters by status credit_pending`() {
      ...
      stubAllCredits(creditPending, credited)
    ...
    fun `filters by status refund_pending`() {
      ...
      val credited = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      stubAllCredits(refundPending, credited)
    ...
    fun `filters by status refunded`() {
      ...
      val credited = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      stubAllCredits(refunded, credited)
    ...
    fun `CRD-034 filters by status failed`() {
      ...
      val credited = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      stubAllCredits(failed, credited)
    ...
    fun `CRD-035 invalid status returns empty set when no matching credits`() {
      stubAllCredits(createCredit(id = 1, resolution = CreditResolution.CREDITED))
    ...
    fun `CRD-040 filters by prison`() {
      ...
      stubCompletedCredits(lei, mdi)

      val result = creditService.listCredits(prisons = listOf("LEI"))

      assertThat(result).hasSize(1)
      assertThat(result[0].prison?.nomisId).isEqualTo("LEI")
    }
    ...
    fun `CRD-042 filters by prison__isnull=true`() {
      ...
      stubCompletedCredits(noPrison, withPrison)
    ...
    fun `CRD-041 filters by multiple prison IDs`() {
      ...
      stubCompletedCredits(lei, mdi, bxi)

      val result = creditService.listCredits(prisons = listOf("LEI", "MDI"))

      assertThat(result).hasSize(2)
      assertThat(result.map { it.prison?.nomisId }).containsExactlyInAnyOrder("LEI", "MDI")
    }
    ...
    fun `CRD-043 filters by prison region`() {
      ...
      val leiPrison = Prison().apply {
        nomisId = "LEI"
        region = "Yorkshire and Humber"
      }
      val mdiPrison = Prison().apply {
        nomisId = "MDI"
        region = "Yorkshire and Humber"
      }
    ...
    fun `CRD-044 filters by prison category`() {
      ...
      val catB = PrisonCategory().apply {
        id = 1L
        name = "Category B"
      }
      val leiPrison = Prison().apply {
        nomisId = "LEI"
        categories = mutableSetOf(catB)
      }
    ...
    fun `CRD-045 filters by prison population`() {
      ...
      val adult = PrisonPopulation().apply {
        id = 1L
        name = "Adult"
      }
      val leiPrison = Prison().apply {
        nomisId = "LEI"
        populations = mutableSetOf(adult)
      }
      val mdiPrison = Prison().apply {
        nomisId = "MDI"
        populations = mutableSetOf(adult)
    ...
    fun `CRD-083 filters by resolution`() {
      ...
      assertThat(result).hasSize(1)
      assertThat(result[0].resolution).isEqualTo(CreditResolution.CREDITED.value)
    }
    ...
    fun `CRD-085 filters by received_at__gte and received_at__lt`() {
      ...
      val result = creditService.listCredits(
        receivedAtGte = now.atOffset(ZoneOffset.UTC),
        receivedAtLt = now.plusDays(1).atOffset(ZoneOffset.UTC),
      )
    ...
    fun `CRD-036 filter valid=true returns credit_pending or credited`() {
      ...
      stubAllCredits(creditPending, credited, refundPending)
    ...
    fun `CRD-060 filters by sender_name case-insensitive on transaction sender_name`() {
      ...
      c1.transaction = makeTransaction(senderName = "John Smith", credit = c1)
      val c2 = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      c2.transaction = makeTransaction(senderName = "Jane Doe", credit = c2)
      stubCompletedCredits(c1, c2)
    ...
    fun `CRD-060 filters by sender_name case-insensitive on payment cardholder_name`() {
      val c1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      c1.payment = makePayment(cardholderName = "Alice Jones", credit = c1)
      val c2 = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      c2.payment = makePayment(cardholderName = "Bob Brown", credit = c2)
      stubCompletedCredits(c1, c2)
    ...
    fun `CRD-060 sender_name matches across transaction and payment`() {
      ...
      c1.transaction = makeTransaction(senderName = "John Smith", credit = c1)
      val c2 = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      c2.payment = makePayment(cardholderName = "John Doe", credit = c2)
      stubCompletedCredits(c1, c2)
    ...
    fun `CRD-061 filters by sender_sort_code exact match`() {
      val c1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      c1.transaction = makeTransaction(senderSortCode = "112233", credit = c1)
      val c2 = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      c2.transaction = makeTransaction(senderSortCode = "445566", credit = c2)
      stubCompletedCredits(c1, c2)
    ...
    fun `CRD-062 filters by sender_account_number exact match`() {
      val c1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      c1.transaction = makeTransaction(senderAccountNumber = "12345678", credit = c1)
      val c2 = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      c2.transaction = makeTransaction(senderAccountNumber = "87654321", credit = c2)
      stubCompletedCredits(c1, c2)
    ...
    fun `CRD-063 filters by sender_roll_number exact match`() {
      val c1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      c1.transaction = makeTransaction(senderRollNumber = "ROLL001", credit = c1)
      val c2 = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      c2.transaction = makeTransaction(senderRollNumber = "ROLL002", credit = c2)
      stubCompletedCredits(c1, c2)
    ...
    fun `CRD-064 filters by sender_name__isblank=true for blank transaction sender_name`() {
      val c1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      c1.transaction = makeTransaction(senderName = "", credit = c1)
      val c2 = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      c2.transaction = makeTransaction(senderName = "John", credit = c2)
      val c3 = createCredit(id = 3, resolution = CreditResolution.CREDITED)
      c3.transaction = makeTransaction(credit = c3) // default empty
      stubCompletedCredits(c1, c2, c3)

      val result = creditService.listCredits(senderNameIsBlank = true)

      // c1 and c3 both have empty senderName
      assertThat(result.map { it.id }).containsExactlyInAnyOrder(1L, 3L)
    }
    ...
    fun `CRD-065 filters by sender_sort_code__isblank=true for blank sort code`() {
      val c1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      c1.transaction = makeTransaction(senderSortCode = "", credit = c1)
      val c2 = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      c2.transaction = makeTransaction(senderSortCode = "112233", credit = c2)
      val c3 = createCredit(id = 3, resolution = CreditResolution.CREDITED)
      c3.transaction = makeTransaction(credit = c3)
      stubCompletedCredits(c1, c2, c3)
    ...
    fun `CRD-066 filters by sender_email case-insensitive substring on payment email`() {
      val c1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      c1.payment = makePayment(email = "John@Example.com", credit = c1)
      val c2 = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      c2.payment = makePayment(email = "jane@other.com", credit = c2)
      stubCompletedCredits(c1, c2)
    ...
    fun `CRD-067 filters by sender_ip_address exact match on payment ip_address`() {
      val c1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      c1.payment = makePayment(ipAddress = "192.168.1.1", credit = c1)
      val c2 = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      c2.payment = makePayment(ipAddress = "10.0.0.1", credit = c2)
      stubCompletedCredits(c1, c2)
    ...
    fun `CRD-068 filters by card_number_first_digits exact match`() {
      val c1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      c1.payment = makePayment(cardNumberFirstDigits = "411111", credit = c1)
      val c2 = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      c2.payment = makePayment(cardNumberFirstDigits = "522222", credit = c2)
      stubCompletedCredits(c1, c2)
    ...
    fun `CRD-069 filters by card_number_last_digits exact match`() {
      val c1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      c1.payment = makePayment(cardNumberLastDigits = "1234", credit = c1)
      val c2 = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      c2.payment = makePayment(cardNumberLastDigits = "5678", credit = c2)
      stubCompletedCredits(c1, c2)
    ...
    fun `CRD-070 filters by card_expiry_date exact match`() {
      val c1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      c1.payment = makePayment(cardExpiryDate = "12/25", credit = c1)
      val c2 = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      c2.payment = makePayment(cardExpiryDate = "06/26", credit = c2)
      stubCompletedCredits(c1, c2)
    ...
    fun `CRD-071 filters by sender_postcode normalized matching`() {
      val addr1 = makeBillingAddress("SW1A 1AA")
      val addr2 = makeBillingAddress("EC2A 4NE")
      val c1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      c1.payment = makePayment(billingAddress = addr1, credit = c1)
      val c2 = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      c2.payment = makePayment(billingAddress = addr2, credit = c2)
      stubCompletedCredits(c1, c2)
    ...
    fun `CRD-071 sender_postcode handles spaces and case differences`() {
      val addr1 = makeBillingAddress("sw1a1aa")
      val c1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      c1.payment = makePayment(billingAddress = addr1, credit = c1)
      stubCompletedCredits(c1)
    ...
    fun `CRD-072 filters by payment_reference prefix match on payment uuid`() {
      ...
      val c1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      c1.payment = makePayment(uuid = uuid1, credit = c1)
      val c2 = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      c2.payment = makePayment(uuid = uuid2, credit = c2)
      stubCompletedCredits(c1, c2)
    ...
    fun `CRD-073 filters by source=bank_transfer returns credits with transactions`() {
      val c1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      c1.transaction = makeTransaction(credit = c1)
      val c2 = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      c2.payment = makePayment(credit = c2)
      val c3 = createCredit(id = 3, resolution = CreditResolution.CREDITED)
      stubCompletedCredits(c1, c2, c3)
    ...
    fun `CRD-086 filters by logged_at__gte truncated to UTC date`() {
      val c1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      c1.logs.add(makeLog(LogAction.CREDITED, c1, LocalDateTime.of(2024, 3, 14, 23, 59)))
      val c2 = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      c2.logs.add(makeLog(LogAction.CREDITED, c2, LocalDateTime.of(2024, 3, 15, 10, 0)))
      val c3 = createCredit(id = 3, resolution = CreditResolution.CREDITED)
      c3.logs.add(makeLog(LogAction.CREDITED, c3, LocalDateTime.of(2024, 3, 16, 8, 0)))
      stubCompletedCredits(c1, c2, c3)

      val result = creditService.listCredits(loggedAtGte = LocalDateTime.of(2024, 3, 15, 0, 0).atOffset(ZoneOffset.UTC))
    ...
    fun `CRD-086 filters by logged_at__lt truncated to UTC date`() {
      ...
      val result = creditService.listCredits(loggedAtLt = LocalDateTime.of(2024, 3, 15, 0, 0).atOffset(ZoneOffset.UTC))
    ...
    fun `CRD-086 logged_at filter uses date truncation ignoring time component`() {
      val c1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      c1.logs.add(makeLog(LogAction.CREDITED, c1, LocalDateTime.of(2024, 3, 15, 0, 0)))
      val c2 = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      c2.logs.add(makeLog(LogAction.CREDITED, c2, LocalDateTime.of(2024, 3, 15, 23, 59)))
      stubCompletedCredits(c1, c2)

      val result = creditService.listCredits(
        loggedAtGte = LocalDateTime.of(2024, 3, 15, 0, 0).atOffset(ZoneOffset.UTC),
        loggedAtLt = LocalDateTime.of(2024, 3, 16, 0, 0).atOffset(ZoneOffset.UTC),
      )
    ...
    fun `CRD-087 filters security_check__isnull=true returns credits without security check`() {
      val c1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      val c2 = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      c2.securityCheck = makeSecurityCheck(c2)
      stubCompletedCredits(c1, c2)
    ...
    fun `CRD-088 filters security_check__actioned_by__isnull=true returns checks not yet actioned`() {
      val c1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      c1.securityCheck = makeSecurityCheck(c1, actionedBy = null)
      val c2 = createCredit(id = 2, resolution = CreditResolution.CREDITED)
      c2.securityCheck = makeSecurityCheck(c2, actionedBy = makeUser("admin1"))
      val c3 = createCredit(id = 3, resolution = CreditResolution.CREDITED)
      stubCompletedCredits(c1, c2, c3)
  ...
  inner class TransitionResolution {
    ...
    fun `transitions credit resolution and saves`() {
      ...
      assertThat(result.resolution).isEqualTo(CreditResolution.CREDITED.value)
      verify(creditRepository).save(credit)
    }

    @Test
    fun `throws exception for invalid transition`() {
      // CreditService.transitionResolution -> Credit.transitionResolution extension
      // which throws InvalidCreditResolutionException (the entity-package one).
      ...
      }.isInstanceOf(InvalidCreditResolutionException::class.java)
    }
  ...
  inner class SearchFullText {
    ...
    fun `search matches transaction sender_name (case insensitive)`() {
      ...
      credit1.transaction = makeTransaction(senderName = "Alice Sender", credit = credit1)
    ...
    fun `search matches payment cardholder_name (case insensitive)`() {
      val credit1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      credit1.payment = makePayment(cardholderName = "Bob Cardholder", credit = credit1)
    ...
    fun `search matches payment UUID prefix (8 chars)`() {
      ...
      val credit1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      credit1.payment = makePayment(uuid = uuid, cardholderName = "Test", credit = credit1)
  ...
  inner class SimpleSearch {
    ...
    fun `simple_search matches payment email (case insensitive)`() {
      val credit1 = createCredit(id = 1, resolution = CreditResolution.CREDITED)
      credit1.payment = makePayment(email = "test@example.com", credit = credit1)
  ...
  inner class OrderingByCreatedReceivedAmount {
    ...
    fun `ordering by created ascending`() {
      val credit1 = createCredit(id = 1, resolution = CreditResolution.CREDITED).apply {
        created = LocalDateTime.of(2024, 1, 1, 10, 0).atOffset(ZoneOffset.UTC)
      }
      val credit2 = createCredit(id = 2, resolution = CreditResolution.CREDITED).apply {
        created = LocalDateTime.of(2024, 1, 2, 10, 0).atOffset(ZoneOffset.UTC)
      }
  ...
  inner class CreditPrisoners {
    ...
    fun `CRD-111 processes credit_pending credits`() {
      ...
      whenever(logRepository.save(any())).thenReturn(Log())
    ...
    fun `CRD-113 sets resolution to CREDITED and owner to requesting user`() {
      ...
      whenever(logRepository.save(any())).thenReturn(Log())
      whenever(userRepository.findByUsername("clerk1")).thenReturn(makeUser("clerk1"))
      ...
      assertThat(saved.resolution).isEqualTo(CreditResolution.CREDITED.value)
      assertThat(saved.owner?.username).isEqualTo("clerk1")
    }
    ...
    fun `CRD-114 creates log entry with LogAction CREDITED and userId`() {
      ...
      whenever(creditRepository.findByIdInWithLock(listOf(1L))).thenReturn(listOf(credit))
      whenever(creditRepository.save(any())).thenReturn(credit)
      whenever(logRepository.save(logCaptor.capture())).thenReturn(Log())
      whenever(userRepository.findByUsername("clerk1")).thenReturn(makeUser("clerk1"))
      ...
      assertThat(log.action).isEqualTo(LogAction.CREDITED.value)
      assertThat(log.user?.username).isEqualTo("clerk1")
  ...
  inner class SetManual {
    ...
    fun `CRD-122 sets resolution to MANUAL and owner to requesting user`() {
      ...
      val saved = captor.firstValue
      assertThat(saved.resolution).isEqualTo(CreditResolution.MANUAL.value)
      assertThat(saved.owner?.username).isEqualTo("clerk1")
    }
    ...
    fun `CRD-123 creates log entry with LogAction MANUAL and userId`() {
      ...
      val log = logCaptor.firstValue
      assertThat(log.action).isEqualTo(LogAction.MANUAL.value)
      assertThat(log.user?.username).isEqualTo("clerk1")
      assertThat(log.credit).isEqualTo(credit)
    }
  ...
  inner class Review {
    ...
    fun `CRD-132 creates REVIEWED log entry for each credit`() {
      ...
      whenever(logRepository.save(logCaptor.capture())).thenReturn(Log())
      whenever(userRepository.findByUsername("security_staff")).thenReturn(makeUser("security_staff"))

      creditService.review(listOf(1L, 2L), "security_staff")

      assertThat(logCaptor.allValues).hasSize(2)
      logCaptor.allValues.forEach { log ->
        assertThat(log.action).isEqualTo(LogAction.REVIEWED.value)
        assertThat(log.user?.username).isEqualTo("security_staff")
      }
  ...
  inner class Refund {
    ...
    fun `CRD-142 sets resolution to REFUNDED for eligible credit`() {
      ...
      assertThat(captor.firstValue.resolution).isEqualTo(CreditResolution.REFUNDED.value)
    }
    ...
    fun `CRD-143 creates REFUNDED log entry with user`() {
      ...
      whenever(creditRepository.save(any())).thenReturn(credit)
      whenever(logRepository.save(logCaptor.capture())).thenReturn(Log())
      whenever(userRepository.findByUsername("bank_admin")).thenReturn(makeUser("bank_admin"))

      creditService.refund(listOf(1L), "bank_admin")

      assertThat(logCaptor.firstValue.action).isEqualTo(LogAction.REFUNDED.value)
      assertThat(logCaptor.firstValue.user?.username).isEqualTo("bank_admin")
    ...
    fun `CRD-143 creates REFUNDED log entry for each credit in batch`() {
      ...
      assertThat(logCaptor.allValues).hasSize(2)
      logCaptor.allValues.forEach { log ->
        assertThat(log.action).isEqualTo(LogAction.REFUNDED.value)
        assertThat(log.user?.username).isEqualTo("bank_admin")
      }
    }
