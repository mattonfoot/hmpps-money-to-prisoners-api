...
import uk.gov.justice.digital.hmpps.moneytoprisonersapi.jpa.entities.FileDownload as FileDownloadEntity

@ExtendWith(MockitoExtension::class)
@DisplayName("FileDownloadsResource")
class FileDownloadsResourceTest {

  @Mock
  private lateinit var fileDownloadService: FileDownloadService

  @InjectMocks
  private lateinit var resource: FileDownloadsResource

  private fun makeDownload(
    id: Long = 1L,
    label: String = "bank_statement",
    date: LocalDate = LocalDate.of(2024, 1, 15),
  ) = FileDownloadEntity().apply {
    this.id = id
    this.label = label
    this.date = date
  }
  ...
  inner class CreateFileDownload {
    ...
    fun `COR-002 returns 201 with created download record`() {
      ...
      val dto = response.body as? FileDownload
