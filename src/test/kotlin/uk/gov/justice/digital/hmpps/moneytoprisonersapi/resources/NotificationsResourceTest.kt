...
import uk.gov.justice.digital.hmpps.moneytoprisonersapi.jpa.entities.NotificationCreditevent
import uk.gov.justice.digital.hmpps.moneytoprisonersapi.jpa.entities.NotificationPrisonerprofileevent
...
class NotificationsResourceTest {
  ...
  @InjectMocks
  private lateinit var notificationResource: NotificationsResource
  ...
  private fun makeEvent(
    id: Long = 1L,
    rule: String = "MONP",
    triggeredAt: OffsetDateTime = OffsetDateTime.of(2024, 1, 15, 10, 0, 0, 0, ZoneOffset.UTC),
    credit: Credit? = null,
    ...
    if (credit != null) {
      event.creditEvent = NotificationCreditevent().apply {
        this.credit = credit
        this.event = event
      }
    }
    if (prisonerProfile != null) {
      event.prisonerProfileEvent = NotificationPrisonerprofileevent().apply {
  ...
  inner class ListEvents {
    ...
    fun `NOT-003 returns events for current user including global events`() {
      val userEvent = makeEvent(id = 1)
      val globalEvent = makeEvent(id = 2)
