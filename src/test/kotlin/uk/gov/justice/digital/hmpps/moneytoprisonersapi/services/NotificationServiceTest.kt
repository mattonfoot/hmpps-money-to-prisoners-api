...
class NotificationServiceTest {
  ...
  private fun authUser(username: String = "user1") = AuthUser().apply {
    this.id = 42L
  ...
  inner class GetEmailFrequency {
    ...
    fun `NOT-010 returns frequency when preference exists`() {
      val prefs = EmailNotificationPreferences().apply {
        user = authUser()
        frequency = EmailFrequency.DAILY.value
      }
      whenever(emailPreferencesRepository.findByUserUsername("user1")).thenReturn(prefs)
    ...
    fun `NOT-010 returns never when no preference set`() {
      whenever(emailPreferencesRepository.findByUserUsername("user1")).thenReturn(null)
  ...
  inner class SetEmailFrequency {
    ...
    fun `NOT-012 creates new preference when none exists`() {
      val owner = authUser()
      whenever(emailPreferencesRepository.findByUserUsername("user1")).thenReturn(null)
      whenever(userRepository.findByUsername("user1")).thenReturn(owner)
      ...
      verify(emailPreferencesRepository).save(
        org.mockito.kotlin.argThat<EmailNotificationPreferences> { prefs ->
          prefs.user?.username == "user1" && prefs.frequency == EmailFrequency.DAILY.value
        },
      )
    }
    ...
    fun `NOT-012 updates existing preference`() {
      val existing = EmailNotificationPreferences().apply {
        id = 1L
        user = authUser()
        frequency = EmailFrequency.NEVER.value
      }
      whenever(emailPreferencesRepository.findByUserUsername("user1")).thenReturn(existing)

      notificationService.setEmailFrequency("user1", EmailFrequency.DAILY)

      assertThat(existing.frequency).isEqualTo(EmailFrequency.DAILY.value)
