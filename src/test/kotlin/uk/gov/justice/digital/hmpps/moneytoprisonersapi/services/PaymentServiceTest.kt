...
class PaymentServiceTest {
  ...
  private fun makeCredit(resolution: CreditResolution = CreditResolution.INITIAL): Credit = Credit().apply {
    amount = 1000L
  ...
  inner class CreatePayment {
    ...
    fun `should create payment with pending status and INITIAL credit`() {
      ...
      val savedPayment = Payment().apply {
        amount = 1000
        status = "pending"
        credit = savedCredit
      }
      ...
      assertThat(capturedCredit.resolution).isEqualTo(CreditResolution.INITIAL.value)
  ...
  inner class UpdatePayment {
    ...
    fun `should throw not found for missing payment`() {
      ...
      whenever(paymentRepository.findById(uuid)).thenReturn(Optional.empty())
    ...
    fun `should throw conflict when payment status is not pending`() {
      ...
      whenever(paymentRepository.findById(uuid)).thenReturn(Optional.of(payment))
    ...
    fun `should transition credit to PENDING when payment status becomes taken`() {
      ...
      whenever(paymentRepository.findById(uuid)).thenReturn(Optional.of(payment))
      whenever(creditRepository.save(any<Credit>())).thenReturn(credit)
      ...
      assertThat(savedCredit.resolution).isEqualTo(CreditResolution.PENDING.value)
    ...
    fun `should leave credit as INITIAL when payment status becomes failed`() {
      ...
      verify(creditRepository, never()).save(any<Credit>())
    ...
    fun `should set credit resolution to FAILED when payment status becomes rejected`() {
      ...
      val creditCaptor = argumentCaptor<Credit>()
      verify(creditRepository).save(creditCaptor.capture())
      assertThat(creditCaptor.firstValue.resolution).isEqualTo(CreditResolution.FAILED.value)
    }
    ...
    fun `should create billing address on first PATCH`() {
      ...
      val savedAddress = BillingAddress().apply {
        id = 1L
      ...
      whenever(paymentRepository.findById(uuid)).thenReturn(Optional.of(payment))
      whenever(billingAddressRepository.save(any<BillingAddress>())).thenReturn(savedAddress)
      whenever(paymentRepository.save(any<Payment>())).thenReturn(payment)
    ...
    fun `should update existing billing address in-place`() {
      val uuid = UUID.randomUUID()
      val credit = makeCredit(CreditResolution.INITIAL)
      val existingAddress = BillingAddress().apply {
        id = 5L
        line1 = "Old Street"
        city = "Old City"
      }
      ...
      whenever(paymentRepository.findById(uuid)).thenReturn(Optional.of(payment))
      whenever(billingAddressRepository.save(any<BillingAddress>())).thenReturn(existingAddress)
      whenever(paymentRepository.save(any<Payment>())).thenReturn(payment)
    ...
    fun `should set explicit received_at when transitioning to taken`() {
      ...
      val credit = makeCredit(CreditResolution.INITIAL)
      val payment = makePayment(status = "pending", credit = credit)
      val explicitReceivedAt = OffsetDateTime.of(2024, 1, 15, 10, 30, 0, 0, ZoneOffset.UTC)
  ...
  inner class ListPayments {
    ...
    fun `should list only pending payments`() {
      ...
      whenever(paymentRepository.findByStatus("pending")).thenReturn(listOf(pendingPayment))
    ...
    fun `should filter pending payments by modified_lt`() {
      val cutoff = OffsetDateTime.of(2024, 1, 10, 0, 0, 0, 0, ZoneOffset.UTC)
      val credit = makeCredit(CreditResolution.INITIAL)
      val payment = makePayment(status = "pending", credit = credit)

      whenever(paymentRepository.findByStatusAndModifiedBefore("pending", cutoff)).thenReturn(listOf(payment))
  ...
  inner class ReconcilePayments {
    ...
    fun `should create batch with taken payments in date range`() {
      ...
      val credit = makeCredit(CreditResolution.PENDING).apply {
        receivedAt = OffsetDateTime.of(2024, 1, 15, 10, 0, 0, 0, ZoneOffset.UTC)
      }

      whenever(
        creditRepository.findByResolutionAndReconciledFalseAndReceivedAtGreaterThanEqualAndReceivedAtBefore(
          eq(CreditResolution.PENDING.value),
          eq(from),
          eq(to),
        ),
      ).thenReturn(listOf(credit))
      whenever(paymentBatchRepository.findMaxRefCode()).thenReturn(null)
      whenever(paymentBatchRepository.save(any<PaymentBatch>())).thenAnswer { invocation -> invocation.arguments[0] }
    ...
    fun `should return null when no payments in date range`() {
      ...
      verify(paymentBatchRepository, never()).save(any<PaymentBatch>())
    }
    ...
    fun `should auto-increment ref_code from max plus 1`() {
      ...
      whenever(paymentBatchRepository.findMaxRefCode()).thenReturn("5")
      whenever(paymentBatchRepository.save(any<PaymentBatch>())).thenAnswer { invocation -> invocation.arguments[0] }

      paymentService.reconcilePayments(request)

      val batchCaptor = argumentCaptor<PaymentBatch>()
