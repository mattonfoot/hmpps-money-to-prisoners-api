...
class TransactionTest {
  ...
  private fun createTransaction(
    ...
    if (refCode != null) this.refCode = refCode else this.refCode = null
  }
  ...
  inner class SenderInfo {
    ...
    @Test
    fun `sender_roll_number defaults to empty string`() {
      val txn = createTransaction(senderRollNumber = "ROLL123")
      assertEquals("ROLL123", txn.senderRollNumber)

      // Django CharField(blank=True) — Kotlin entity mirrors with empty string,
      // not null. The factory converts null → "".
      val txnEmpty = createTransaction(senderRollNumber = null)
      assertEquals("", txnEmpty.senderRollNumber)
    }

    @Test
    fun `sender fields collapse null to empty string`() {
      ...
      assertEquals("", txn.senderSortCode)
      assertEquals("", txn.senderAccountNumber)
      assertEquals("", txn.senderName)
      assertEquals("", txn.senderRollNumber)
    }
  }
  ...
  inner class IncompleteSenderInfo {
    ...
    fun `incomplete_sender_info can be set to true`() {
      ...
      assertTrue(txn.incompleteSenderInfo)
    }
  }
  ...
  inner class ReferenceInSenderField {
    ...
    fun `reference_in_sender_field can be set to true`() {
      ...
      assertTrue(txn.referenceInSenderField)
    }
  }
  ...
  inner class Timestamps {
    ...
    @Test
    fun `created and modified are auto-populated`() {
      // Django auto_now_add lives on @PrePersist; the entity gives them a
      // default so they're never null pre-save.
      val txn = createTransaction()
