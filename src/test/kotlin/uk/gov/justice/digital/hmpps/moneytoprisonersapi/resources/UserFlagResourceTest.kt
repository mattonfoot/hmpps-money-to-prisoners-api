...
@DisplayName("UsersResource flag endpoints")
class UserFlagResourceTest {
  ...
  private fun makeFlag(user: MtpUser, name: String) = UserFlag(id = 10L, user = user, name = name)
  ...
  inner class CreateFlag {
    ...
    fun `AUTH-030 creates flag for user`() {
      ...
      whenever(userService.findByUsername("testuser")).thenReturn(user)
      whenever(userFlagRepository.existsByUserAndName(user, "new_flag")).thenReturn(false)
    ...
    fun `AUTH-030 flag is unique per user - returns 400 on duplicate`() {
      val user = makeUser()
      whenever(userService.findByUsername("testuser")).thenReturn(user)
      whenever(userFlagRepository.existsByUserAndName(user, "existing_flag")).thenReturn(true)
    ...
    fun `saves flag with correct user and name`() {
      val user = makeUser()
      whenever(userService.findByUsername("testuser")).thenReturn(user)
      whenever(userFlagRepository.existsByUserAndName(user, "test_flag")).thenReturn(false)
      ...
      assertThat(captor.firstValue.name).isEqualTo("test_flag")
    }
  }
  ...
  inner class DeleteFlag {
    ...
    fun `AUTH-032 deletes flag by name`() {
      ...
      whenever(userService.findByUsername("testuser")).thenReturn(user)
      whenever(userFlagRepository.findByUserAndName(user, "to_delete")).thenReturn(flag)
    ...
    fun `returns 404 when flag not found`() {
      val user = makeUser()
      whenever(userService.findByUsername("testuser")).thenReturn(user)
      whenever(userFlagRepository.findByUserAndName(user, "missing")).thenReturn(null)
