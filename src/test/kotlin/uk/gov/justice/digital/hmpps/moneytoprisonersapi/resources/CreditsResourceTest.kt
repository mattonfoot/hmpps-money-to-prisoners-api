...
class CreditsResourceTest {
  ...
  @Suppress("unused")
  @Mock
  private lateinit var commentRepository: uk.gov.justice.digital.hmpps.moneytoprisonersapi.jpa.repositories.CommentRepository

  @Suppress("unused")
  @Mock
  private lateinit var creditRepository: uk.gov.justice.digital.hmpps.moneytoprisonersapi.jpa.repositories.CreditRepository

  @Suppress("unused")
  @Mock
  private lateinit var userRepository: uk.gov.justice.digital.hmpps.moneytoprisonersapi.jpa.repositories.AuthUserRepository

  @InjectMocks
  private lateinit var creditResource: CreditsResource
  ...
  private fun createCredit(
    ...
    attachTransaction: Boolean = false,
  ): CreditEntity = CreditEntity().apply {
    this.id = id
    this.amount = amount
    this.prisonerNumber = prisonerNumber
    this.prisonerName = "John Smith"
    this.prisonerDob = LocalDate.of(1990, 1, 15)
    this.prison = prisonNomisId?.let { nomis -> PrisonPrison().apply { this.nomisId = nomis } }
    this.resolution = resolution.value
    this.blocked = blocked
    this.receivedAt = OffsetDateTime.of(2024, 3, 15, 10, 30, 0, 0, ZoneOffset.UTC)
    // `incompleteSenderInfo` is a column on TransactionTransaction, not on the
    // credit itself. Attach a transaction when the test needs the flag.
    if (incompleteSenderInfo || attachTransaction) {
  ...
  inner class ListCredits {
    ...
    fun `CRD-020 returns paginated response with computed status`() {
      val credits = listOf(
        createCredit(id = 1, prisonNomisId = "LEI", resolution = CreditResolution.PENDING),
    ...
    fun `CRD-015 credit_pending status included in response`() {
      val credits = listOf(createCredit(prisonNomisId = "LEI", resolution = CreditResolution.PENDING, blocked = false))
      whenever(creditService.listCredits()).thenReturn(credits)
    ...
    fun `CRD-017 refund_pending status included in response`() {
      val credits = listOf(createCredit(prisonNomisId = null, resolution = CreditResolution.PENDING))
      whenever(creditService.listCredits()).thenReturn(credits)
    ...
    fun `passes received_at date range to service`() {
      val gte = OffsetDateTime.of(2024, 1, 1, 0, 0, 0, 0, ZoneOffset.UTC)
      val lt = OffsetDateTime.of(2024, 2, 1, 0, 0, 0, 0, ZoneOffset.UTC)
    ...
    fun `CRD-086 passes logged_at__gte and logged_at__lt filters to service`() {
      val gte = OffsetDateTime.of(2024, 3, 15, 0, 0, 0, 0, ZoneOffset.UTC)
      val lt = OffsetDateTime.of(2024, 3, 16, 0, 0, 0, 0, ZoneOffset.UTC)
  ...
  inner class ReviewCredits {
    ...
    fun `CRD-136 returns 204 when review succeeds`() {
      val request = ReviewRequest(creditIds = listOf(1L))
    ...
    fun `CRD-130 returns 400 for empty list`() {
      val request = ReviewRequest(creditIds = emptyList())
    ...
    fun `CRD-130 passes credit ids and user to service`() {
      val request = ReviewRequest(creditIds = listOf(1L, 2L))
